...
export async function checkEvmHolderGate(
  wallet: string,
  ...
  try {
    ...
    const meetsGate = Number(balance) >= (minBalance ?? 1);
    return meetsGate
      ? { eligible: true, reason: `Holds ${balance.toString()} token(s) of the required collection.` }
      : {
          eligible: false,
          reason: `Requires holding at least ${minBalance} token(s) of ${tokenAddress}.`,
          details: { balance: balance.toString() },
        };
  } catch {
    return { eligible: false, reason: "Could not read holder-gate contract — check the contract address." };
  }
}
